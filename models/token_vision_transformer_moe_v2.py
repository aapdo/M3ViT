...
class TokenBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        if moe:
            ...
            # ShareabilityPredictor - determines shared vs task-specific
            self.share_pred = ShareabilityPredictor(d_model=dim, d_task_emb=gate_task_specific_dim, temperature=1.0, hard=False)
    ...
    # LN2 + MoE + Residual
    # ==========================================================================
    def transition_stage(self, outs: dict, prev_state: "RouterState|None",
        ...
        Args:
            outs: {task_id: [B, N, C]}
            prev_state: RouterState or None (첫 블록이면 None)
        ...
        Returns:
            ...
            shared_x:    [B, N, C] 대표 토큰 (공유 위치만 유효; 공유 위치 없으면 None)
        ...
        # -------- (3) shared_x — A-2 soft mixing --------
        # 공유 위치가 하나도 없으면 shared_x = None
        if not valid_shared.any():
            shared_x = None
        else:
            X = torch.stack([outs[t] for t in range(T)], dim=0)  # [T, B, N, C]

            # 점수 × 마스크로 가중치 계산
            GM = G * M.float()                          # [T, B, N]
            W_sum = GM.sum(dim=0, keepdim=True) + eps   # [1, B, N]
            W = GM / W_sum                              # [T, B, N] (softmax-like)

            shared_x = (W.unsqueeze(-1) * X).sum(dim=0)  # [B, N, C]
        ...
        return shared_bits, shared_x, trans_aux
    ...
    def apply_shared_broadcast(
        ...
        *,
        ...
        Args:
            ...
            shared_x: Tensor[B,N,C] representative token
            inplace: if True, write into outs[t] tensors in-place (may break autograd if views)
                    default False uses torch.where assignment pattern.
        ...
        assert shared_x.shape == (B, N, C), f"shared_x shape {shared_x.shape} != {(B,N,C)}"

        for t in range(T):
            mask_t = ((shared_bits >> t) & 1).bool()  # [B,N]
            if not mask_t.any():
                continue

            if inplace:
                # In-place masked assignment
                outs[t] = outs[t].clone()  # safer than writing into potential views
                outs[t][mask_t] = shared_x[mask_t]
            else:
                outs[t] = torch.where(mask_t.unsqueeze(-1), shared_x, outs[t])
    ...
    def forward(
        ...
        NOTE:
        - 아래 forward는 "구조"를 먼저 잡기 위한 것으로,
        transition_stage / apply_broadcast / shared_subpath_fwd 등은 TODO로 남겨둠.
        """
        # -------------------------
        # 0) aux init
        # -------------------------
        ...
        if hasattr(self, "share_pred") and self.share_pred is not None:
            ...
            # TODO: share_pred가 없으면 디버그/백워드 호환용 기본값(전부 TS 또는 전부 SH 정책) 선택
        ...
        # 입력:
        #   - outs: {t: [B,N,C]}
        #   - prev_state: RouterState(shared_bits/shared_x) or None
        #   - g_shared: {t: [B,N]}
        ...
        if hasattr(self, "transition_stage") and self.transition_stage is not None:
            shared_bits, shared_x, trans_aux = self.transition_stage(
                outs=outs,
                prev_state=prev_state,
                g_shared=g_shared,
                # TODO: allow_partial_split 
            )
        ...
        if shared_x is not None:
            if hasattr(self, "apply_shared_broadcast") and self.apply_shared_broadcast is not None:
                outs = self.apply_shared_broadcast(outs=outs, shared_bits=shared_bits, shared_x=shared_x)
            else:
        ...
        if not self.moe:
            # ===== Dense Block =====
            # TODO:
            # - shared_x가 있으면 shared 위치에 대해 "dense 1회 실행" 후 broadcast
            # - TS 위치는 각 task별 dense MLP 실행(기존 dense_mlp_stage 재사용 가능하나,
            #   shared 위치는 compute에서 제외하는 마스킹 버전이 이상적)
            #
            # 빠른 구조:
            #  (A) per-task dense_mlp_stage로 전체 수행 후
            #  (B) shared_x를 별도로 dense 1회 수행하고 shared 위치를 overwrite(broadcast)
            #
            outs = self.dense_mlp_stage(outs)

            if shared_x is not None:
                # TODO: shared_dense_ffn_1x 구현(권장: LN2+MLP+residual을 shared_x에 적용)
                # 여기서는 existing dense path의 한 스텝을 재활용하려면 별도 함수가 필요:
                #   shared_out = shared_x + drop_path( mlp( norm2(shared_x) ) )
                if hasattr(self, "shared_dense_forward") and self.shared_dense_forward is not None:
                    shared_out = self.shared_dense_forward(shared_x)
                else:
                    # 최소 동작: 동일한 연산을 shared_x에 적용 (dense 블록의 self.mlp 사용)
                    # WARNING: 이 코드는 "전체를 shared 1회 실행" 구조는 맞지만,
                    #          실제론 LN2/MLP/residual 세부가 dense 블록과 동일함을 전제.
                    shared_out = shared_x + self.drop_path(self.mlp(self.norm2(shared_x)))

                # broadcast overwrite
                for t in range(self.num_tasks):
                    mask_t = ((shared_bits >> t) & 1).bool()
                    if mask_t.any():
                        outs[t] = torch.where(mask_t.unsqueeze(-1), shared_out, outs[t])

            # state 갱신
            curr_state = self._state_for_next_block({
                "curr_bits": shared_bits,
                "selector_tbN": None,  # TODO: 필요하면 기록
            })
            # TODO: RouterState에 shared_x를 추가했다면 여기서 넣어야 함
            if hasattr(curr_state, "shared_x"):
                curr_state.shared_x = shared_x if shared_x is not None else None
            ...
            # ===== MoE Block =====
            # TODO:
            # - SH 위치: B-1 shared_ffn(shared_x) 1회 실행 후 broadcast
            # - TS 위치: 기존 moe_stage 수행 (compute_mask에서 SH 제외)
            #
            # 1) TS MoE 실행
            # TODO: moe_stage에 shared_bits를 넘겨서, compute_mask를 TS로 제한하도록 수정 필요
            #   - compute_mask = task_specific_mask & (~shared_mask_for_task)
            #   - shared_mask_for_task = ((shared_bits>>t)&1)
            router_out = None
            moe_aux = None

            if hasattr(self, "router_stage") and hasattr(self, "moe_stage"):
                # 기존 router_stage는 "게이트 공유/재사용" 중심이라
                # Persistent Sharing에서는 router_stage가 꼭 필요하지 않을 수 있음.
                # TODO: TS-only MoE routing을 위해 router_stage/moe_stage를 단순화하거나,
                #       moe_stage가 직접 TS compute_mask를 받아 처리하도록 변경.
                ...
                # TODO: router_out에 shared_bits를 넣거나, moe_stage에 인자로 전달
                router_out["shared_bits"] = shared_bits

                outs, moe_aux = self.moe_stage(outs, router_out)
                aux = self._merge_aux(aux, moe_aux)
            else:
                # TODO: moe_stage 미구현이면 no-op
                pass

            # 2) SH shared_ffn 1회 실행 + overwrite broadcast
            if shared_x is not None:
                # TODO: self.shared_ffn(B-1) must exist in moe blocks
                if hasattr(self, "shared_ffn") and self.shared_ffn is not None:
                    # shared path는 "대표 토큰" 기준 1회 실행
                    # (설계에 따라 LN+Residual 포함 여부 결정)
                    # 여기선 shared_ffn이 "FFN body"라고 가정하면 residual을 포함해 적용:
                    shared_out = shared_x + self.drop_path(self.shared_ffn(self.norm2(shared_x)))
                else:
                    # TODO: 반드시 구현 필요
                    raise RuntimeError("moe=True block requires self.shared_ffn for B-1 shared path")
