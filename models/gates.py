"""Gating mechanisms for Mixture-of-Experts models.

This module provides base and concrete implementations of gating functions
that determine how inputs are routed to different experts in MoE layers.
"""
...
class BaseGate(ABC, nn.Module):
    """Base class for MoE gating mechanisms.

    All gate implementations should inherit from this class and implement
    the forward method to compute expert routing decisions.
    """

    def __init__(self, num_experts, world_size=1):
        """Initialize base gate.

        Args:
            num_experts: Number of experts per device
            world_size: Number of devices (for distributed training)
        """
        super().__init__()
        self.num_experts = num_experts
        self.world_size = world_size
        # Note: FMoE에서는 전체 expert 수를 num_experts * world_size로 설정하지만,
        # 본 코드에서는 expert를 노드별로 분리하지 않으므로 num_experts만 사용함.
        # self.tot_expert = num_experts * world_size

        self.tot_expert = num_experts
        self.loss = None
    ...
    def forward(self, inp, *args, **kwargs):
        ...
        Args:
            inp: Input tensor
            *args, **kwargs: Additional arguments specific to gate implementation

        Returns:
            top_k_indices: Indices of selected experts
            top_k_gates: Gate values for selected experts
        """
        raise NotImplementedError

    def set_loss(self, loss):
        """Accumulate auxiliary loss for load balancing."""
    ...
    def get_loss(self, clear=True):
        """Get accumulated loss and optionally clear it."""
        loss = self.loss
        if clear:
            self.loss = None
        return loss
...
class NoisyGate(BaseGate):
    def __init__(self, d_model, num_expert, world_size, top_k=2, no_noise=False, return_decoupled_activation=False,regu_experts_fromtask=False,\
        ...
        self.disable_aux_loss_update_on_forward = False
    ...
    def forward(self, inp):
        ...
        if self.training and not self.disable_aux_loss_update_on_forward:
            ...
            loss = cv_imp + cv_load
            logger.debug(f"NoisyGate_VMoE: cv_imp={cv_imp:.4f}, cv_load={cv_load:.4f}, importance={importance.tolist()}, load={load.tolist()}")
            self.set_loss(loss)
...
class NoisyGate_VMoE(BaseGate):
    def __init__(self, d_model, num_expert=16, world_size=2, top_k=4, noise_std=0, no_noise=False,
