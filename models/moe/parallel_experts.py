...
class ParallelExperts(nn.Module):
    ...
    # Class variable to track if init has been logged
    _init_logged = False
    ...
    def reset_parameters(self) -> None:
        # Use Truncated Normal initialization for expert weights
        # This matches the ViT initialization style (trunc_normal with std=0.02)
        # Truncated normal clips values to [mean-2*std, mean+2*std] range
        from timm.layers import trunc_normal_
        trunc_normal_(self.w, std=.02)
        if not ParallelExperts._init_logged:
            import logging
            logger = logging.getLogger(__name__)
            logger.info(f"[Init] ParallelExperts: trunc_normal_(std=0.02) - logging once, applies to all instances")
            ParallelExperts._init_logged = True
